...
const DEFAULT_CATEGORIES = [
  ...
  'Health', 'Shopping', 'Utilities', 'Education', 'SIP', 'Insurance', 'Other'
];
...
function seedDefaultData() {
  if (DB.get('seeded_v1', false)) return;
  if (loadPolicies().length === 0) {
    // Axis Max Life Term — ₹50L coverage, ₹948/month, 10yr payment, 60yr coverage
    addPolicy(
      'Axis Max Life Term', 'Axis Max Life', 'term',
      5000000, 948, 'monthly',
      '2026-06-12',   // start date
      '2086-06-12',   // coverage end (60 years)
      '2036-06-12'    // payment end (10 years)
    );
  }
  DB.set('seeded_v1', true);
}
...
function switchSubtab(subtab) {
  ...
  const debtsTab = document.getElementById('tab-debts');
  debtsTab.querySelectorAll('.subtab-btn').forEach(el => el.classList.remove('active'));
  debtsTab.querySelectorAll('.subtab-content').forEach(el => el.classList.remove('active'));
  debtsTab.querySelector('.subtab-btn[data-subtab="' + subtab + '"]').classList.add('active');
  document.getElementById('subtab-' + subtab).classList.add('active');
}

function switchSavingsSubtab(stab) {
  currentSavingsSubtab = stab;
  document.querySelectorAll('.savings-subtab-btn').forEach(el => el.classList.remove('active'));
  document.querySelectorAll('.savings-subtab-content').forEach(el => el.classList.remove('active'));
  document.querySelector('.savings-subtab-btn[data-stab="' + stab + '"]').classList.add('active');
...
function openFabMenu() {
  ...
  const isSavings = currentTab === 'savings';
  document.getElementById('fab-add-txn').style.display       = isSavings ? 'none' : '';
  document.getElementById('fab-add-debt').style.display      = isSavings ? 'none' : '';
  document.getElementById('fab-add-sip').style.display       = isSavings ? '' : 'none';
  document.getElementById('fab-add-insurance').style.display = isSavings ? '' : 'none';
}
...
// ─────────────────────────────────────────────
//  STORAGE: SIPs & INSURANCE
// ─────────────────────────────────────────────
function loadSIPs()         { return DB.get('sips', []); }
function saveSIPs(d)        { DB.set('sips', d); }
function loadPolicies()     { return DB.get('policies', []); }
function savePolicies(d)    { DB.set('policies', d); }
function loadSIPPayments()  { return DB.get('sipPayments', []); }
function saveSIPPayments(d) { DB.set('sipPayments', d); }
function loadInsPayments()  { return DB.get('insPayments', []); }
function saveInsPayments(d) { DB.set('insPayments', d); }

// ─────────────────────────────────────────────
//  SIP & INSURANCE HELPERS
// ─────────────────────────────────────────────
function sipFreqLabel(f)  { return { daily:'Daily', weekly:'Weekly', monthly:'Monthly' }[f] || f; }
function insFreqLabel(f)  { return { monthly:'Monthly', quarterly:'Quarterly', 'half-yearly':'Half-Yearly', yearly:'Yearly' }[f] || f; }
function insTypeLabel(t)  { return { term:'Term Life', health:'Health', vehicle:'Vehicle', other:'Other' }[t] || t; }
function insTypeIcon(t)   { return { term:'🛡️', health:'💊', vehicle:'🚗', other:'📋' }[t] || '📋'; }

function fmtCoverage(amount) {
  if (!amount) return '—';
  if (amount >= 10000000) return CURRENCY + parseFloat((amount / 10000000).toFixed(2)).toString() + ' Cr';
  if (amount >= 100000)   return CURRENCY + parseFloat((amount / 100000).toFixed(2)).toString() + ' L';
  return fmt(amount);
}

function calcSIPExpected(sip) {
  const start = new Date(sip.startDate + 'T00:00:00');
  const today = new Date(); today.setHours(0, 0, 0, 0);
  if (start > today) return 0;
  const days = Math.floor((today - start) / 86400000);
  if (sip.frequency === 'daily')   return (days + 1) * sip.amount;
  if (sip.frequency === 'weekly')  return (Math.floor(days / 7) + 1) * sip.amount;
  // monthly
  const months = (today.getFullYear() - start.getFullYear()) * 12 + (today.getMonth() - start.getMonth()) + 1;
  return Math.max(0, months) * sip.amount;
}

function calcSIPActual(sipId) {
  return loadSIPPayments().filter(p => p.sipId === sipId).reduce((s, p) => s + p.amount, 0);
}

function getInsNextDue(pol) {
  if (!pol.startDate) return null;
  const start = new Date(pol.startDate + 'T00:00:00');
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const end   = pol.endDate ? new Date(pol.endDate + 'T00:00:00') : null;
  let next = new Date(start);
  while (next <= today) {
    if      (pol.frequency === 'monthly')     next.setMonth(next.getMonth() + 1);
    else if (pol.frequency === 'quarterly')   next.setMonth(next.getMonth() + 3);
    else if (pol.frequency === 'half-yearly') next.setMonth(next.getMonth() + 6);
...
function daysUntilLabel(days) {
  if (days === null)  return { text: 'Expired',   cls: 'due-expired' };
  ...
  if (days <= 30)     return { text: 'In ' + days + 'd', cls: 'due-near' };
...
//  SIP CRUD
// ─────────────────────────────────────────────
function addSIP(name, amount, frequency, startDate, note) {
  const sips = loadSIPs();
  sips.unshift({ id: uid(), name: name.trim(), amount: parseFloat(amount), frequency, startDate, note: note || '' });
  saveSIPs(sips);
}
function deleteSIP(id) {
  saveSIPs(loadSIPs().filter(s => s.id !== id));
  saveSIPPayments(loadSIPPayments().filter(p => p.sipId !== id));
}
function logSIPPayment(sipId, amount, date, note) {
  const payments = loadSIPPayments();
  payments.unshift({ id: uid(), sipId, amount: parseFloat(amount), date, note: note || '' });
  saveSIPPayments(payments);
}

// ─────────────────────────────────────────────
//  INSURANCE CRUD
// ─────────────────────────────────────────────
function addPolicy(name, insurer, type, sumAssured, premium, frequency, startDate, endDate, paymentEndDate) {
  const pols = loadPolicies();
  pols.unshift({ id: uid(), name: name.trim(), insurer: insurer.trim(), type,
    sumAssured: parseFloat(sumAssured) || 0, premium: parseFloat(premium),
    frequency, startDate, endDate: endDate || '', paymentEndDate: paymentEndDate || '' });
  savePolicies(pols);
}
function deletePolicy(id) {
  savePolicies(loadPolicies().filter(p => p.id !== id));
  saveInsPayments(loadInsPayments().filter(p => p.polId !== id));
}

// ─────────────────────────────────────────────
//  RENDER: SAVINGS
// ─────────────────────────────────────────────
function renderSavings() { renderSIPs(); renderInsurance(); }

function renderSIPs() {
  const sips  = loadSIPs();
  const list  = document.getElementById('sip-list');
  const empty = document.getElementById('sip-empty');
  if (!list) return;
  if (!sips.length) { list.innerHTML = ''; empty.classList.add('show'); return; }
  empty.classList.remove('show');
  list.innerHTML = sips.map(sipCardHTML).join('');
}

function sipCardHTML(sip) {
  const expected = calcSIPExpected(sip);
  const actual   = calcSIPActual(sip.id);
  const annual   = sip.frequency === 'daily' ? sip.amount * 365
    : sip.frequency === 'weekly' ? sip.amount * 52 : sip.amount * 12;
  const progress = expected > 0 ? Math.min(100, Math.round((actual / expected) * 100)) : 0;
  const behind   = expected - actual;
  const statusCls  = behind <= 0 ? 'due-ok'      : 'due-overdue';
  const statusText = behind <= 0 ? '✓ Up to date' : 'Behind ' + fmt(behind);
  return '<div class="invest-card">'
    + '<div class="invest-header">'
    + '<div class="invest-icon-wrap sip-icon">📈</div>'
    + '<div class="invest-info">'
    + '<div class="invest-name">' + escHtml(sip.name) + '</div>'
    + '<div class="invest-meta">' + fmt(sip.amount) + ' <span class="invest-freq">' + sipFreqLabel(sip.frequency) + '</span>'
    + (sip.note ? ' · ' + escHtml(sip.note) : '') + '</div>'
    + '</div>'
    + '<div class="invest-top-actions">'
    + '<button class="sip-log-btn btn-log" data-sip-id="' + escAttr(sip.id) + '" data-sip-name="' + escAttr(sip.name) + '" data-sip-amount="' + escAttr(String(sip.amount)) + '">+ Log</button>'
    + '<button class="invest-del btn-del" data-id="' + escAttr(sip.id) + '" data-kind="sip" title="Delete">✕</button>'
    + '</div>'
    + '</div>'
    + (expected > 0
      ? '<div class="invest-progress-wrap"><div class="invest-progress-bar"><div class="invest-progress-fill" style="width:' + progress + '%"></div></div><span class="invest-progress-pct">' + progress + '%</span></div>'
      : '')
    + '<div class="invest-stats-row">'
    + '<div class="invest-stat"><div class="istat-val accent-val">' + fmt(expected) + '</div><div class="istat-lbl">Expected</div></div>'
    + '<div class="invest-stat-div"></div>'
    + '<div class="invest-stat"><div class="istat-val green-val">' + fmt(actual) + '</div><div class="istat-lbl">Logged</div></div>'
    + '<div class="invest-stat-div"></div>'
    + '<div class="invest-stat"><div class="istat-val">' + fmt(annual) + '</div><div class="istat-lbl">Per Year</div></div>'
    + '</div>'
    + '<div class="invest-footer">'
    + '<span class="invest-since">Since ' + fmtDate(sip.startDate) + '</span>'
    + '<span class="due-badge ' + statusCls + '">' + statusText + '</span>'
    + '</div>'
    + '</div>';
}
...
function renderInsurance() {
  ...
  if (!list) return;
  if (!pols.length) { list.innerHTML = ''; empty.classList.add('show'); return; }
  empty.classList.remove('show');
  list.innerHTML = pols.map(insuranceCardHTML).join('');
}

function insuranceCardHTML(pol) {
  const nextDue = getInsNextDue(pol);
  const days    = daysUntil(nextDue);
  const due     = daysUntilLabel(days);
  const annual  = pol.frequency === 'monthly'     ? pol.premium * 12
    : pol.frequency === 'quarterly'   ? pol.premium * 4
    : pol.frequency === 'half-yearly' ? pol.premium * 2 : pol.premium;
  const paidSoFar = loadInsPayments().filter(p => p.polId === pol.id).reduce((s, p) => s + p.amount, 0);
  return '<div class="invest-card ins-card">'
    + '<div class="invest-header">'
    + '<div class="invest-icon-wrap ins-icon">' + insTypeIcon(pol.type) + '</div>'
    + '<div class="invest-info">'
    + '<div class="invest-name">' + escHtml(pol.name) + '</div>'
    + '<div class="invest-meta">' + escHtml(pol.insurer) + ' <span class="invest-freq">' + insTypeLabel(pol.type) + '</span></div>'
    + '</div>'
    + '<div class="invest-top-actions">'
    + '<button class="ins-log-btn btn-log" data-pol-id="' + escAttr(pol.id) + '" data-pol-name="' + escAttr(pol.name) + '" data-pol-amount="' + escAttr(String(pol.premium)) + '">+ Log</button>'
    + '<button class="invest-del btn-del" data-id="' + escAttr(pol.id) + '" data-kind="insurance" title="Delete">✕</button>'
    + '</div>'
    + '</div>'
    + '<div class="ins-coverage-strip">'
    + '<span class="ins-cov-label">🛡️ Coverage</span>'
    + '<span class="ins-cov-val">' + fmtCoverage(pol.sumAssured) + '</span>'
    + '</div>'
    + '<div class="invest-stats-row">'
    + '<div class="invest-stat"><div class="istat-val">' + fmt(pol.premium) + '</div><div class="istat-lbl">' + insFreqLabel(pol.frequency) + '</div></div>'
    + '<div class="invest-stat-div"></div>'
    + '<div class="invest-stat"><div class="istat-val red-val">' + fmt(annual) + '</div><div class="istat-lbl">Annual Cost</div></div>'
    + '<div class="invest-stat-div"></div>'
    + '<div class="invest-stat"><div class="istat-val green-val">' + fmt(paidSoFar) + '</div><div class="istat-lbl">Paid Total</div></div>'
    + '</div>'
    + '<div class="invest-footer">'
    + '<span class="invest-since">'
    + (pol.paymentEndDate ? 'Pay till ' + fmtDate(pol.paymentEndDate) : (pol.endDate ? 'Until ' + fmtDate(pol.endDate) : 'No end date'))
    + '</span>'
...
// ─────────────────────────────────────────────
//  MODAL: LOG INSURANCE PREMIUM
// ─────────────────────────────────────────────
function openInsLogModal(polId, polName, polAmount) {
  document.getElementById('ins-log-id').value      = polId;
  document.getElementById('ins-log-sub').textContent = polName;
  document.getElementById('ins-log-amount').value  = polAmount;
  document.getElementById('ins-log-date').value    = todayISO();
  document.getElementById('ins-log-note').value    = '';
  document.getElementById('ins-log-csym').textContent = loadCurrency();
  openModal('modal-ins-log');
  setTimeout(() => document.getElementById('ins-log-amount').focus(), 200);
}
function saveInsLogModal() {
  const polId  = document.getElementById('ins-log-id').value;
  const polName = document.getElementById('ins-log-sub').textContent;
  const amount = parseFloat(document.getElementById('ins-log-amount').value);
  const date   = document.getElementById('ins-log-date').value || todayISO();
  const note   = document.getElementById('ins-log-note').value.trim();
  if (!polId)               { showToast('Something went wrong.'); return; }
  if (!amount || amount <= 0) { showToast('Please enter a valid amount.'); return; }
  logInsurancePremium(polId, amount, date, note);
  addTransaction('expense', amount, 'Insurance', 'Premium: ' + polName, date);
  closeModal('modal-ins-log');
  showToast('Premium logged!');
  renderInsurance();
  renderDashboard();
  if (currentTab === 'transactions') renderTransactions();
}
function logInsurancePremium(polId, amount, date, note) {
  const payments = loadInsPayments();
  payments.unshift({ id: uid(), polId, amount: parseFloat(amount), date, note: note || '' });
  saveInsPayments(payments);
}
...
function openAddSIPModal() {
  ...
  document.getElementById('sip-frequency').value  = 'monthly';
  document.getElementById('sip-start-date').value = todayISO();
  document.getElementById('sip-note').value       = '';
  document.getElementById('sip-currency-sym').textContent = loadCurrency();
  openModal('modal-sip');
  setTimeout(() => document.getElementById('sip-name').focus(), 200);
}
function saveSIPModal() {
  ...
  const startDate = document.getElementById('sip-start-date').value || todayISO();
  const note      = document.getElementById('sip-note').value.trim();
  if (!name)             { showToast('Please enter a SIP name.'); return; }
  if (!amount || amount <= 0) { showToast('Please enter a valid amount.'); return; }
  addSIP(name, amount, frequency, startDate, note);
  closeModal('modal-sip');
  showToast('SIP added!');
  renderSIPs();
}
...
function openAddInsuranceModal() {
  ...
  document.getElementById('ins-insurer').value         = '';
  document.getElementById('ins-type').value            = 'term';
  document.getElementById('ins-sum-assured').value     = '';
  document.getElementById('ins-premium').value         = '';
  document.getElementById('ins-frequency').value       = 'yearly';
  document.getElementById('ins-start-date').value      = todayISO();
  document.getElementById('ins-payment-end').value     = '';
  document.getElementById('ins-end-date').value        = '';
  document.getElementById('ins-csym1').textContent     = loadCurrency();
  document.getElementById('ins-csym2').textContent     = loadCurrency();
  openModal('modal-insurance');
  setTimeout(() => document.getElementById('ins-name').focus(), 200);
}
function saveInsuranceModal() {
  ...
  const frequency     = document.getElementById('ins-frequency').value;
  const startDate     = document.getElementById('ins-start-date').value || todayISO();
  const paymentEndDate = document.getElementById('ins-payment-end').value;
  const endDate       = document.getElementById('ins-end-date').value;
  if (!name)                    { showToast('Please enter a policy name.'); return; }
  if (!insurer)                 { showToast('Please enter the insurer name.'); return; }
  if (!premium || premium <= 0) { showToast('Please enter a valid premium amount.'); return; }
  addPolicy(name, insurer, type, sumAssured, premium, frequency, startDate, endDate, paymentEndDate);
...
// ─────────────────────────────────────────────
//  MODAL: LOG SIP PAYMENT
// ─────────────────────────────────────────────
function openSIPLogModal(sipId, sipName, sipAmount) {
  document.getElementById('sip-log-id').value     = sipId;
  document.getElementById('sip-log-sub').textContent = sipName;
  document.getElementById('sip-log-amount').value = sipAmount;
  document.getElementById('sip-log-date').value   = todayISO();
  document.getElementById('sip-log-note').value   = '';
  document.getElementById('sip-log-csym').textContent = loadCurrency();
  openModal('modal-sip-log');
  setTimeout(() => document.getElementById('sip-log-amount').focus(), 200);
}
function saveSIPLogModal() {
  const sipId  = document.getElementById('sip-log-id').value;
  const amount = parseFloat(document.getElementById('sip-log-amount').value);
  const date   = document.getElementById('sip-log-date').value || todayISO();
  const note   = document.getElementById('sip-log-note').value.trim();
  if (!sipId)               { showToast('Something went wrong.'); return; }
  if (!amount || amount <= 0) { showToast('Please enter a valid amount.'); return; }
  const sipName = document.getElementById('sip-log-sub').textContent;
  logSIPPayment(sipId, amount, date, note);
  addTransaction('expense', amount, 'SIP', 'SIP: ' + sipName, date);
  closeModal('modal-sip-log');
...
function exportData() {
  const data = {
    ...
    persons:      loadPersons(),
    sips:         loadSIPs(),
    sipPayments:  loadSIPPayments(),
    policies:     loadPolicies(),
    insPayments:  loadInsPayments()
  };
...
function clearAllData() {
  ['transactions','debts','settlements','categories','persons','sips','sipPayments','policies','insPayments','seeded_v1'].forEach(k => DB.remove(k));
  CURRENCY = loadCurrency();
  renderDashboard();
  renderSettings();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('fab-add-sip').addEventListener('click', () => { closeFabMenu(); openAddSIPModal(); });
  document.getElementById('fab-add-insurance').addEventListener('click', () => { closeFabMenu(); openAddInsuranceModal(); });
  ...
  // DEBT SUBTABS
  document.querySelectorAll('#tab-debts .subtab-btn').forEach(btn => {
    btn.addEventListener('click', () => switchSubtab(btn.dataset.subtab));
  });

  // SAVINGS SUBTABS
  document.querySelectorAll('.savings-subtab-btn').forEach(btn => {
    btn.addEventListener('click', () => switchSavingsSubtab(btn.dataset.stab));
  });
  ...
  document.querySelector('.app-main').addEventListener('click', e => {
    ...
    // Log SIP payment
    const logBtn = e.target.closest('.sip-log-btn');
    if (logBtn) {
      e.stopPropagation();
      openSIPLogModal(logBtn.dataset.sipId, logBtn.dataset.sipName, logBtn.dataset.sipAmount);
    }

    // Log Insurance premium
    const insLogBtn = e.target.closest('.ins-log-btn');
    if (insLogBtn) {
      e.stopPropagation();
      openInsLogModal(insLogBtn.dataset.polId, insLogBtn.dataset.polName, insLogBtn.dataset.polAmount);
    }

    // Delete SIP or Insurance
    const delInv = e.target.closest('.invest-del');
    if (delInv) {
      e.stopPropagation();
      const kind = delInv.dataset.kind;
      const id   = delInv.dataset.id;
      if (kind === 'sip') {
        openConfirm('Delete SIP', 'Delete this SIP and all its logged payments?', () => {
          deleteSIP(id); renderSIPs(); showToast('SIP deleted.');
        });
      } else if (kind === 'insurance') {
        openConfirm('Delete Policy', 'Delete this insurance policy?', () => {
          deletePolicy(id); renderInsurance(); showToast('Policy deleted.');
        });
      }
    }
  ...
  // SIP MODAL
  document.getElementById('sip-cancel').addEventListener('click', () => closeModal('modal-sip'));
  document.getElementById('sip-save').addEventListener('click', saveSIPModal);
  document.getElementById('modal-sip').addEventListener('click', e => {
    if (e.target === e.currentTarget) closeModal('modal-sip');
  });

  // INSURANCE MODAL
  document.getElementById('ins-cancel').addEventListener('click', () => closeModal('modal-insurance'));
  document.getElementById('ins-save').addEventListener('click', saveInsuranceModal);
  document.getElementById('modal-insurance').addEventListener('click', e => {
    if (e.target === e.currentTarget) closeModal('modal-insurance');
  });

  // INSURANCE LOG MODAL
  document.getElementById('ins-log-cancel').addEventListener('click', () => closeModal('modal-ins-log'));
  document.getElementById('ins-log-save').addEventListener('click', saveInsLogModal);
  document.getElementById('modal-ins-log').addEventListener('click', e => {
    if (e.target === e.currentTarget) closeModal('modal-ins-log');
  });

  // SIP LOG MODAL
  document.getElementById('sip-log-cancel').addEventListener('click', () => closeModal('modal-sip-log'));
  document.getElementById('sip-log-save').addEventListener('click', saveSIPLogModal);
  document.getElementById('modal-sip-log').addEventListener('click', e => {
    if (e.target === e.currentTarget) closeModal('modal-sip-log');
  });
